  ...
  <div className={`${colorPalate.background} ${colorPalate.text}`} style={{height:'100vh'}}>
      ...
      >
        ...
        <div className="flex flex-row justify-center gap-2 " style={{width:'100%'}}>
          ...
          {/* tasklist section */}
          <div className={`${colorPalate.foreground} p-2 rounded-lg`} style={{ height: "92vh", width: "20%"  }}>
            {taskListPageData}
          </div>
          {/* tasklist section */}
...
export async function getServerSideProps(ctx) {
  ...
  await dbConnect();
  const data = await fetchHelper(`${NEXT_URL}/api/${session.user.id}/tasklistcategory`, "GET");
