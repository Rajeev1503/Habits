...
const TaskDetails = (props) => {
  ...
  },[taskContext?.task, TaskListContext?.taskList])
...
<div className="p-4">
  ...
  const [displayOptionData, setDisplayOptionData] = useState(<EditTaskComponent />);
  ...
  return (
      <div
        ...
        <div className="flex flex-row justify-between items-center text-center bg-accent-background-dark text-lighttext font-semibold text-xs">
          <div
            className={`${isAtInfo ? 'bg-main-background-dark':''} flex-grow p-1 cursor-pointer`}
            onClick={() => {
        ...
        <br />
        {displayOptionData}
        <p className="text-right cursor-pointer" onClick={()=>{setDisplayExpanded(false)}}>Hide
            </p></div>
      </div>
  );
};
